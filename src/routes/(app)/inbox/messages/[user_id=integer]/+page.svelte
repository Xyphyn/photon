  ...
  <div class="sticky bottom-4 p-4">
    <form
      class={[
        ...
        settings.value.messages.fullMarkdown
          ? 'flex flex-col'
    ...
    >
      {#if settings.value.messages.fullMarkdown}
        <MarkdownEditor
          previewButton={false}
      ...
      {:else}
        <Button
          onclick={() => (settings.value.messages.fullMarkdown = true)}
          size="custom"
          class="h-9 w-9"
          rounding="xl"
        >
      ...
      <div class="flex flex-row gap-2">
        {#if settings.value.messages.fullMarkdown}
          <Button
            onclick={() => (settings.value.messages.fullMarkdown = false)}
            size="custom"
            class="h-9 w-9"
            rounding="xl"
          >
