<script lang="ts">
  ...
  import { settings, SettingsData } from '$lib/app/settings.svelte'
  ...
  let userHost = $state<UserDataUi>({
    profile: new Profile(data.userHost.meta),
    settings: new SettingsData(data.userHost.settings).value,
    site: data.userHost.site,
  })

  const user = setContext<UserDataUi>('user', userHost)
  ...
  if (browser) {
    $effect(() => {
      if (settings.value) {
        ...
        document.documentElement.classList.add(
          settings.value.font == 'inter'
            ? 'font-inter'
            : settings.value.font == 'system'
              ? 'font-system'
              : 'font-sans',
        )
      }
    ...
    $effect(() => {
      document.documentElement.dir =
        ($locale == 'he' || $locale == 'ar') && settings.value.useRtl
          ? 'rtl'
          : 'ltr'
    })
  }
    ...
    {#key user.site}
      {#if page.data.slots?.sidebar?.component}
        ...
        <SvelteComponent
          ...
          class={[c, 'p-3 sm:p-6']}
        />
      {:else if user.site?.site_view}
        <InstanceCard
          site={user.site.site_view}
          taglines={user.site.taglines}
          admins={user.site.admins}
          version={user.site.version}
          class={[c, 'p-3 sm:p-6']}
        />
      {:else}
