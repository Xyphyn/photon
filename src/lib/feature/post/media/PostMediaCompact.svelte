<script lang="ts">
  import { t } from '$lib/app/i18n'
  import { settings, type View } from '$lib/app/settings.svelte'
  import { showImage } from '$lib/ui/generic/ExpandableImage.svelte'
  import { Button, modal } from 'mono-svelte'
  import {
    Calendar,
    DocumentText,
    ExclamationTriangle,
    Icon,
    Link,
    Photo,
    PresentationChartBar,
    VideoCamera,
  } from 'svelte-hero-icons/dist'
  import { bestImageURL, postLink } from '../post-helpers'
  import type { PostModel } from '../post.svelte'

  const thumbnailSize = (view: View) =>
    view == 'compact' ? 'w-22 h-22 sm:w-28' : 'w-24 h-24 sm:w-32'

  interface Props {
    post: PostModel
    view?: View
    blur?: boolean
    style?: string
    class?: string
  }

  let {
    post,
    view = 'cozy',
    blur = post.filterRule != 'none',
    style = '',
    class: clazz = '',
  }: Props = $props()

  let size = $derived(thumbnailSize(view))
</script>

<!--
  @component
  Thumbnails for compact and list view posts.
-->
<div class={[size, 'relative group/media', clazz]} {style} role="presentation">
  <svelte:element
    this={!settings.expandImages || post.mediaType != 'image' ? 'a' : 'button'}
    href={postLink(post.post)}
    aria-label={post.mediaType == 'image'
      ? $t('aria.postDecor.openImage', { default: post.post.name })
      : $t('aria.postDecor.openPost', { default: post.post.name })}
    onclick={() => {
      if (post.mediaType == 'image') showImage(bestImageURL(post.post, false, -1))
    }}
    role={post.mediaType == 'image' ? 'button' : 'link'}
    tabindex="0"
    class="cursor-pointer h-full block"
  >
    <div
      class={[
        'relative overflow-hidden rounded-2xl max-h-full h-full',
        'btn-secondary hover-scale-effect',
      ]}
    >
      {#if post.post.thumbnail_url || post.mediaType == 'image'}
        {@const thumbnail = post.post.thumbnail_url != undefined && post.mediaType != 'image'}
        <picture class="rounded-[inherit]">
          <!--I would add AVIF, but lemmy.world's AVIF is broken as of currently-->
          {#each ['webp'] as format}
            <source
              srcset="{bestImageURL(
                post.post,
                thumbnail,
                128,
                format as 'avif' | 'webp',
              )} 1x, {bestImageURL(
                post.post,
                thumbnail,
                256,
                format as 'avif' | 'webp',
              )} 2x, {bestImageURL(post.post, thumbnail, 512, format as 'avif' | 'webp')} 3x"
              media="(min-width: 0px)"
              type="image/{format}"
            />
          {/each}
          <img
            src={blur ? '' : bestImageURL(post.post, thumbnail, -1, null)}
            loading="lazy"
            class={['object-cover relative overflow-hidden rounded-[inherit] h-full', size]}
            alt={post.post.alt_text ?? ' '}
            class:blur-xl={blur}
          />
        </picture>
        {#if post.mediaType != 'image'}
          <div class="post-media-indicator">
            <Icon src={post.mediaType == 'iframe' ? VideoCamera : Link} micro size="16" />
          </div>
        {/if}
      {:else}
        {@const iconMap = new Map([
          ['embed', Link],
          ['iframe', VideoCamera],
          ['poll', PresentationChartBar],
          ['event', Calendar],
        ])}
        <div
          class={[
            ' w-full h-full rounded-xl grid place-items-center',
            'text-slate-600 dark:text-zinc-400',
          ]}
        >
          <Icon src={iconMap.get(post.mediaType) ?? DocumentText} solid size="32" />
        </div>
      {/if}
    </div>
    {#if blur}
      <Icon
        src={ExclamationTriangle}
        solid
        size="32"
        class="absolute w-8 h-8 mx-auto my-auto z-30 inset-0 opacity-30"
      />
    {/if}
  </svelte:element>
  {#if post.post.alt_text}
    <Button
      onclick={() => modal({ title: 'Alt text', body: post.post.alt_text })}
      aria-label="Alt text"
      class="absolute bottom-0 left-0 z-20 m-1"
      size="square-md"
      rounding="xl"
    >
      <Icon src={Photo} size="16" micro />
    </Button>
  {/if}
</div>

<style>
  @reference '../../../../app.css';
  .hover-scale-effect > *,
  .hover-scale-effect > picture > img {
    transition: transform 200ms var(--ease-cubic);
  }
  .hover-scale-effect:hover > *,
  .hover-scale-effect:hover > picture > img {
    transform: scale(95%);
  }
  .hover-scale-effect:active > *,
  .hover-scale-effect:active > picture > img {
    transform: scale(90%);
  }

  .post-media-indicator {
    position: absolute;
    bottom: 0;
    left: 0;
    display: flex;
    align-items: end;
    padding: calc(var(--spacing) * 2);
    height: calc(var(--spacing) * 16);
    width: 100%;
    border-bottom-left-radius: inherit;
    border-bottom-right-radius: inherit;

    background-image: linear-gradient(
      to bottom,
      --alpha(var(--color-slate-50) / 0%),
      --alpha(var(--color-slate-50) / 95%)
    );

    @variant dark {
      background-image: linear-gradient(
        to bottom,
        --alpha(var(--color-zinc-900) / 0%),
        --alpha(var(--color-zinc-900) / 95%)
      );
    }
  }
</style>
